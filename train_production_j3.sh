#!/bin/bash
#SBATCH --job-name=es-prod-j3-24h
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem=0
#SBATCH --time=24:00:00
#SBATCH --output=logs/train_production_j3_%j.out
#SBATCH --error=logs/train_production_j3_%j.err
#SBATCH --partition=workq

set -e
echo "ES-LOBS5 Production Training J3 (24h) + WandB"
echo "Config: H5 base + task_size=10 (n_steps=20, bg_msgs=100)"

mkdir -p logs checkpoints/es_production_j3
...
class Config:
    ...
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_j3'
    checkpoint_every: int = 10; seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_j3'

config = Config()
run = wandb.init(project='ES-LOBS5', name='J3_steps20_bg100_task10', config=asdict(config), tags=['J3', 'production', '24h', 'task10'])
...
print(f'COMPLETE (J3) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
np.save(os.path.join(config.checkpoint_dir, 'fitness_history.npy'), np.array(fitness_history))
wandb.finish()
"
echo "J3 Complete! End: \$(date)"
