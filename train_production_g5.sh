#!/bin/bash
#SBATCH --job-name=es-prod-g5-24h
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem=0
#SBATCH --time=24:00:00
#SBATCH --output=logs/train_production_g5_%j.out
#SBATCH --error=logs/train_production_g5_%j.err
...
mkdir -p logs checkpoints/es_production_g5
...
class Config:
    ...
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_g5'
    checkpoint_every: int = 10; seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_g5'

config = Config()
run = wandb.init(project='ES-LOBS5', name='G5_steps10_bg100_task500', config=asdict(config), tags=['G5', 'production', '24h'])
...
print(f'COMPLETE (G5) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
