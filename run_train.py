...
if __name__ == "__main__":
	...
	parser.add_argument("--debugprint", type=str2bool, default=False,
					help="Enable JAX debug prints (affects XLA compilation - disable for production)")
	...
	# Chunking configuration (now always enabled with n_chunks=5 by default)
	# Can override with JAX_N_CHUNKS environment variable if needed
	MSG_LEN = 24  # Tokens per message/order
	total_seq_len = args.msg_seq_len * MSG_LEN  # e.g., 500 * 24 = 12000
	n_chunks_default = 5
	n_chunks_actual = int(os.environ.get('JAX_N_CHUNKS', str(n_chunks_default)))
	chunk_size = total_seq_len // n_chunks_actual
