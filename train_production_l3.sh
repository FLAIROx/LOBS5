#!/bin/bash
#SBATCH --job-name=es-prod-l3-24h
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem=0
#SBATCH --time=24:00:00
#SBATCH --output=logs/train_production_l3_%j.out
#SBATCH --error=logs/train_production_l3_%j.err
#SBATCH --partition=workq

set -e
echo "ES-LOBS5 Production Training L3 (24h) + WandB + EggRollBS"
echo "Config: H5 base + task_size=10 + file_idx=0 + EggRollBS (baseline subtraction)"

mkdir -p logs checkpoints/es_production_l3
...
class Config:
    ...
    noiser: str = 'eggrollbs'; sigma: float = 0.01; lr: float = 0.001; lora_rank: int = 4; grad_clip: float = 1.0
    n_perturbations: int = 1024; n_epochs: int = 1000; n_steps: int = 20; n_warmup_msgs: int = 500
    ...
    group_size: int = 8  # Required for EggRollBS
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_l3'
    checkpoint_every: int = 10; seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_l3'

config = Config()
run = wandb.init(project='ES-LOBS5', name='L3_steps20_bg100_task10_eggrollbs', config=asdict(config), tags=['L3', 'production', '24h', 'task10', 'fixed_data', 'eggrollbs'])
...
print(f'COMPLETE (L3) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
np.save(os.path.join(config.checkpoint_dir, 'fitness_history.npy'), np.array(fitness_history))
wandb.finish()
"
echo "L3 Complete! End: \$(date)"
