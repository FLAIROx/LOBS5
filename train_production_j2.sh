...
set -e
echo "ES-LOBS5 Production Training J2 (24h) + WandB"
echo "Config: G5 base + task_size=10 (n_steps=10, bg_msgs=100)"

mkdir -p logs checkpoints/es_production_j2
...
config = Config()
run = wandb.init(project='ES-LOBS5', name='J2_steps10_bg100_task10', config=asdict(config), tags=['J2', 'production', '24h', 'task10'])
...
print(f'COMPLETE (J2) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
