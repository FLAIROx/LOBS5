#!/bin/bash
#SBATCH --job-name=es-prod-l4-24h
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem=0
#SBATCH --time=24:00:00
#SBATCH --output=logs/train_production_l4_%j.out
#SBATCH --error=logs/train_production_l4_%j.err
#SBATCH --partition=workq

set -e
echo "ES-LOBS5 Production Training L4 (24h) + WandB + EggRollBS"
echo "Config: I5 base + task_size=10 + file_idx=0 + EggRollBS (baseline subtraction)"

mkdir -p logs checkpoints/es_production_l4
...
class Config:
    ...
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_l4'
    checkpoint_every: int = 10; seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_l4'

config = Config()
run = wandb.init(project='ES-LOBS5', name='L4_steps50_bg100_task10_eggrollbs', config=asdict(config), tags=['L4', 'production', '24h', 'task10', 'fixed_data', 'eggrollbs'])
...
print(f'COMPLETE (L4) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
np.save(os.path.join(config.checkpoint_dir, 'fitness_history.npy'), np.array(fitness_history))
wandb.finish()
"
echo "L4 Complete! End: \$(date)"
