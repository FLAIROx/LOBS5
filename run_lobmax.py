...
if __name__ == "__main__":
    ...
    MODEL_PRESETS = {
        ...
        # MoE Optimization Presets - for tokens_per_expert experiments
        # tokens_per_expert = batch × seq × topk / num_experts
        "1.9B-A153M": {
            # Baseline: 16 experts, topk=1 → tokens_per_expert = 24K
            "d_model": 768, "n_layers": 12, "num_heads": 12, "mlp_dim": 3072,
            "num_experts": 16, "num_experts_per_tok": 1,
            "micro_bsz": 6, "lr_base": 0.0005, "lr_factor": 1,
            "wandb_project": "lobmax-1.9B-A153M"
        },
        "1.0B-A153M-8E": {
            # 8 experts → tokens_per_expert = 48K (2x baseline)
            "d_model": 768, "n_layers": 12, "num_heads": 12, "mlp_dim": 3072,
            "num_experts": 8, "num_experts_per_tok": 1,
            "micro_bsz": 6, "lr_base": 0.0005, "lr_factor": 1,
            "wandb_project": "lobmax-1.0B-A153M-8E"
        },
        "1.0B-A306M-8E-T2": {
            ...
            "micro_bsz": 6, "lr_base": 0.0005, "lr_factor": 1,
            "wandb_project": "lobmax-1.0B-A306M-8E-T2"
        },
        "0.6B-A153M-4E": {
            ...
            "micro_bsz": 6, "lr_base": 0.0005, "lr_factor": 1,
            "wandb_project": "lobmax-0.6B-A153M-4E"
        },
        "0.6B-A306M-4E-T2": {
            ...
            # GPU MoE Optimization (Tokamax GMM)
            "megablox": False,           # Disable TPU block-sparse
            "sparse_matmul": False,      # Disable sparse ops
            "use_tokamax_gmm": True,     # Enable GPU grouped matmul

            "micro_bsz": 6, "lr_base": 0.0005, "lr_factor": 1,
            "wandb_project": "lobmax-0.6B-A306M-4E-T2"
        },
    }
    ...
    parser.add_argument("--use_tokamax_gmm", type=str2bool, default=False,
                        help="Use Tokamax GMM kernel for GPU MoE optimization (disables megablox/sparse_matmul).")

    # === Attention Configuration ===
    parser.add_argument("--attention_kernel", type=str, default="flash",
                        choices=["dot_product", "flash", "cudnn_flash_te", "cudnn_flash_jax", "jax_native"])
    parser.add_argument("--attention_type", type=str, default="global",
                        choices=["global", "local_sliding", "chunk", "mla", "full"],
                        help="Attention type: global (O(n²)), local_sliding (O(n×W)), chunk, mla, full")
    parser.add_argument("--sliding_window_size", type=int, default=0,
                        help="Sliding window size for local attention (0=disabled). Recommended: 1024-2048 for seq_len=12K")
    ...
    parser.add_argument("--remat_policy", type=str, default="full",
                        choices=["full", "minimal", "save_dot_except_mlp",
                                 "qkv_proj_offloaded", "minimal_offloaded"],
                        help="Remat (rematerialization) policy for memory/speed trade-off.")
    parser.add_argument("--quantization", type=str, default="",
                        choices=["", "fp8", "int8", "te_fp8_delayedscaling",
                                 "te_fp8_currentscaling"],
                        help="Quantization mode: fp8, int8, or TE FP8 variants.")
    ...
    print(f"    mlp_dim={args.mlp_dim}")
    print(f"    attention_kernel={args.attention_kernel}, attention_type={args.attention_type}")
    if args.attention_type == "local_sliding" and args.sliding_window_size > 0:
        print(f"    [SLIDING WINDOW] window_size={args.sliding_window_size} (reduces O(n²) to O(n×{args.sliding_window_size}))")
    ...
    if args.num_experts > 1:
        ...
        print(f"    [MoE] sparse_matmul={args.sparse_matmul}, megablox={args.megablox}, use_tokamax_gmm={args.use_tokamax_gmm}")
        print(f"    [MoE] capacity_factor={args.capacity_factor}")
    ...
    # TE shard guard: required for cudnn_flash_te fused attention kernel
    # Without this, TE's DotProductAttention fails with "Global mesh resource is not set"
    if args.attention_kernel == "cudnn_flash_te":
        from transformer_engine.jax.sharding import global_shard_guard, MeshResource
        print("[*] TE global_shard_guard(MeshResource()) activated for cudnn_flash_te")
