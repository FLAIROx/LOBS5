#!/bin/bash
#SBATCH --job-name=es-prod-h5-24h
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem=0
#SBATCH --time=24:00:00
#SBATCH --output=logs/train_production_h5_%j.out
#SBATCH --error=logs/train_production_h5_%j.err
#SBATCH --partition=workq

set -e
echo "ES-LOBS5 Production Training H5 (24h) + WandB"
echo "Config: n_steps=20, bg_msgs/step=100, task_size=500"

mkdir -p logs checkpoints/es_production_h5
...
class Config:
    ...
    checkpoint_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_h5'
    checkpoint_every: int = 10; seed: int = 42
    output_dir: str = '/lus/lfs1aip2/home/s5e/kangli.s5e/AlphaTrade/LOBS5/checkpoints/es_production_h5'

config = Config()
run = wandb.init(project='ES-LOBS5', name='H5_steps20_bg100_task500', config=asdict(config), tags=['H5', 'production', '24h'])
...
print(f'COMPLETE (H5) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
np.save(os.path.join(config.checkpoint_dir, 'fitness_history.npy'), np.array(fitness_history))
wandb.finish()
"
echo "H5 Complete! End: \$(date)"
