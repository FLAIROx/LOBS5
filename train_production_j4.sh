...
set -e
echo "ES-LOBS5 Production Training J4 (24h) + WandB"
echo "Config: I5 base + task_size=10 (n_steps=50, bg_msgs=100)"

mkdir -p logs checkpoints/es_production_j4
...
config = Config()
run = wandb.init(project='ES-LOBS5', name='J4_steps50_bg100_task10', config=asdict(config), tags=['J4', 'production', '24h', 'task10'])
...
print(f'COMPLETE (J4) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
