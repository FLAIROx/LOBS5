...
set -e
echo "ES-LOBS5 Production Training J1 (24h) + WandB"
echo "Config: D5 base + task_size=10 (n_steps=10, bg_msgs=10)"

mkdir -p logs checkpoints/es_production_j1
...
config = Config()
run = wandb.init(project='ES-LOBS5', name='J1_steps10_bg10_task10', config=asdict(config), tags=['J1', 'production', '24h', 'task10'])
...
print(f'COMPLETE (J1) | Epochs: {len(fitness_history)} | Best: {best_fitness:.4f}')
