...
# Import JAX Native Attention (lightweight alternative)
from lobmax.jax_native_attention import create_jax_native_attention
...
class TransformerLayer(nn.Module):
    ...
    def setup(self):
        ...
        attention_type_enum = AttentionType.GLOBAL  # default
        if hasattr(cfg, 'attention_type'):
            attention_type_str = cfg.attention_type.upper()
            print(f"[DEBUG TransformerLayer] cfg.attention_type = '{cfg.attention_type}' (raw)")
            if attention_type_str == "LOCAL_SLIDING":
                attention_type_enum = AttentionType.LOCAL_SLIDING
                print(f"[DEBUG TransformerLayer] Setting attention_type_enum = LOCAL_SLIDING ✓")
            elif attention_type_str == "CHUNK":
                attention_type_enum = AttentionType.CHUNK
            elif attention_type_str == "MLA":
                attention_type_enum = AttentionType.MLA
            elif attention_type_str == "FULL":
                attention_type_enum = AttentionType.FULL
        else:
            print(f"[DEBUG TransformerLayer] cfg has NO attention_type attribute! Using default GLOBAL")

        sliding_window = None
        if hasattr(cfg, 'sliding_window_size') and cfg.sliding_window_size > 0:
            sliding_window = cfg.sliding_window_size
            print(f"[DEBUG TransformerLayer] sliding_window_size = {sliding_window} ✓")
        else:
            print(f"[DEBUG TransformerLayer] sliding_window_size = 0 or not set")

        # Choose attention implementation based on attention_kernel
        if cfg.attention == "jax_native":
            # Use JAX Native Attention (lightweight, no dependencies)
            print(f"[DEBUG TransformerLayer] Using JAX Native Attention with sliding_window={sliding_window}")
            self.attention = create_jax_native_attention(
            ...
            # Use MaxText attention_as_linen (default)
            print(f"[DEBUG TransformerLayer] Using MaxText attention_as_linen with kernel={cfg.attention}")
